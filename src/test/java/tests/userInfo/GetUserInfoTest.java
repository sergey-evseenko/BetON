...
public class GetUserInfoTest extends BaseTest {
    ...
    public void getUserInfoWithExpiredToken() {
        ...
        response = new UserAdapter().getUserInfoWithExpiredToken(expiredToken);
        assertEquals(response.getError(), "invalid_token", "invalid error");
    }

    @Test(description = "get user info without token")
    public void getUserInfoWithoutToken() {
        response = new UserAdapter().getInfoWithoutToken();
        assertEquals(response.getError(), "unauthorized", "invalid error");
    }
}
